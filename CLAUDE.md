...
- **Animation:** `useReveal` (IntersectionObserver scroll reveals) is the default. `gsap` + ScrollTrigger power the Career timeline scroll-fill, and `gsap` Draggable + InertiaPlugin power the **drag-to-scroll Reels shelf** in Projects (`.reels-track`) — all **lazy-imported** so they stay out of the initial bundle. (The Reels drag uses an off-DOM proxy mapped onto `el.scrollLeft`, **not** Draggable's `type:'scrollLeft'`, whose scrollProxy wraps the children in a block `<div>` and breaks the flex row; it's fine-pointer-only — touch scrolls natively — and skipped under reduced motion.) `three` IS used now: the hero centrepiece is a vintage SLR camera GLB (`CameraStage`, vanilla three + DRACOLoader, all lazy-imported, paused offscreen via IntersectionObserver). `motion` was **pruned** (was unused). The Remotion intro was **removed** (2026-06-02) — `remotion`, `@remotion/*`, and `@react-three/fiber` are all gone. `lenis` + `@gsap/react` are still in `package.json` but unused — prune candidates.
...
This work (3D camera hero, CWV fixes, Playwright E2E) was built on `feature/3d-set` and **shipped to production** on 2026-06-01 — merged into `react-version` and promoted via `vercel --prod`. Note: Vercel's "Production Branch" isn't wired to `react-version`, so pushes deploy as *preview*; promote to prod with `vercel --prod` (or set the production branch in the Vercel dashboard).
