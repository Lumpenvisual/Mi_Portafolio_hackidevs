...
// CameraHero — hero with a vintage SLR camera GLTF on the right, mouse/scroll
// parallax, accent lighting and a starfield. Vanilla three.js (no R3F), JS only;
// three is dynamically imported so it stays out of the initial bundle until the
// hero mounts. Degrades to a "Modelo no disponible" message if the model fails.
const MODEL_PATH = '/models/vintage_slr_camera'

export default function CameraHero() {
  const stageRef = useRef(null)
  const pointer = useParallax() // normalized mouse [-1,1], read each frame
  const { model, loading, error } = useModelLoader(MODEL_PATH)
  ...
  return (
    <section className="camera-hero" id="top">
      ...
      {/* RIGHT — 3D stage (canvas is appended here) */}
