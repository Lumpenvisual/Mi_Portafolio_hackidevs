...
const DynamicToolbarExpandable = React.memo(function DynamicToolbarExpandable({
  steps,
  badgeText,
  ...
  const scrollButtonIntoView = useCallback((currentIndex, previousIndex) => {
    ...
    const targetButton = menuContainerRef.current.querySelector(`[data-step-index="${targetIndex}"]`)
  ...
  return (
    <div
      className={cn("space-y-2 w-full max-w-sm sm:max-w-lg mx-auto px-2 sm:px-0", className)}>
      {badgeText && (
        <Badge
          variant="outline"
