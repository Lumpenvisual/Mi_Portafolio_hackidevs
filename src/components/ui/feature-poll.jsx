...
const FeaturePollContext = createContext(null)
...
function FeaturePollRoot({
  ...
  const [selectedArray, setSelectedArray] = useControllableState({
  ...
  const select = useCallback((optionId) => {
  ...
  const isSelected = useCallback((optionId) => selected.includes(optionId), [selected])

  const getPercentage = useCallback((optionId) => {
    if (totalVotes === 0) {
      return 0
    }
...
function FeaturePollOptions({
  ...
  const handleKeyDown = useCallback((event) => {
    ...
    const options = Array.from(
      container.querySelectorAll('[data-slot="feature-poll-option"]:not([disabled])')
    )
